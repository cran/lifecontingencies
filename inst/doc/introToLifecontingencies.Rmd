...
- Exn, pure endowment: $\pureendc{x}{n}=v^2*p_{x,n}$
- axn, annuity: $\ddot{a}_{x} = \sum_{k=0}^{\omega-x} v^{k} * p_{x,k}=\sum_{k=0}^{\omega-x} \ddot{a}_{\lcroof{K+1}} p_{x,k} q_{x+k,1}$
- Axn, life insurance: $\termins{x}{n}=\sum_{k=0}^{n-1} v^{k-1} * p_{x,k} * q_{x+k,1}$
- AExn, endowment: $\insend{x}{n}=\pureendc{x}{n}+\termins{x}{n}$
...
- $\left(n-1\right) * \termins{x}{n} = \lcterm{(IA)}{x}{n} + \lcterm{(DA)}{x}{n}$
