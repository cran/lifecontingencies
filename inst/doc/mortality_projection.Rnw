...
\title{Mortality projection with \textbf{demography} and
\textbf{lifecontingencies} packages}
...
insurances and social security issues. \textbf{demography} and
\textbf{lifecontingencies} packages will be used. The analysis performed within
...
Mortality across any ages is showing continuous reduction across the world.
Moreover, a reliable projection of mortality is very important to accurately
...
Databases via \textbf{demography} package dedicated function. We will use
\textbf{demography} and \textbf{forecast} package sto fit Lee - Carter model and
perform 100 - years in advance extrapolations.\\
Finally, \textbf{lifecontingencies} package will be used to project the cost of a
...
\begin{itemize}
  ...
  annuity of 1 monetary unit. The annuity will be evaluated assuming an
  interest rate of 4\% and an inflation rate of 2\%.
\end{itemize}
...
results.\\
Most of this paper is based on the examples provided in
\cite{rmetrics1} and \cite{charpentierDutang} online manual.
...
and that $\mu_x \sim m_x$, that is the force of mortality to be approximated by the
central rate of mortality \footnote{As shown in \cite{wrightContingencies},
if $p_{x,t}$ were assumed linear between the two consecutive integer ages, 
we could write $m_x = \frac{q_x}{1 - \frac{1}{2}{q_x}}$}.
...
which $t-x=YYYY$. We will perform such exercise on Italy HMD data, saved in
mortalityDataset db.
...
Following code creates a \textbf{demogdata} object from HDM data structure.
...
Plot method is available on \textbf{demogdata}.
...
\textbf{lca} returned object allows us to inspect $a_x$, $b_x$ and $k_t$.
...
We can therefore use \textbf{forecast} package to project the future $k_{t}$s (up to 110).
...
using results from Lee - Carter model. In particular, for ages $0, 1, \ldots, \tau$ on which Lee-Carter model has been fit Equation~\ref{eq:fit1} apply, while for extreme
...
\begin{equation}
\begin{array}{l}
\ln {\hat{\mu_{x,t}}} = a_x + {b_x}{k_t}\\
\hat p_{x,t} = \exp \left(  - \hat {\mu }_{x,t} \right)
\end{array}
\label{eq:fit1}
\end{equation}
