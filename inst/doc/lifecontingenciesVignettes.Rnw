...
%% almost as usual
\author{Giorgio Alfredo Spedicato\\Universit\'a Cattolica del Sacro Cuore}
...
As of December 2011, \pkg{lifecontingencies} seems the first \proglang{R} package that deals with life insurance evaluation.\\
 \proglang{R} has provided many package that actuaries can use within their professional activity.\\
 However most packages are of mainly interest of non-life actuaries, due to the wider importante of 
 regression modelling and distribution fitting in non - life than in life insurance.
The package \pkg{actuar}, \cite{Dutang2008}, provides functions to fit loss distributions and to perform credibility 
analysis. Package \pkg{actuar} represents the computational side of the classical book \cite{klugman2009loss}. The package \pkg{ChainLadder}, \cite{chainLadder}, provides functions to estimate non-life loss reserve.
GLM analysis widely used in predictive modelling can be performed by the \pkg{base} package bundled within \proglang{R}. Specific models 
can be using the \pkg{gamlss} package, \cite{gamlssPkg},  or the \pkg{cplm} package, \cite{cplmPkg}.\\
Life actuaries conversely work more with demographic and financial data. \proglang{R} has a dedicated view to packages dedicated to financial analysis. However 
few packages exist to perform demographic analysis (as \pkg{demography}, \cite{demographyR}, and \pkg{LifeTables}, \cite{LifeTableR}). Finally no package exists that performs 
life contingencies calculations, as of December 2011.\\

Numerous commercial software specifically tailored to actuarial analysis are available in commerce. 
Moses and Prophet are currently the leading actuarial software in life insurance. This package aims to represent the R 
computational support of the concepts developed in the classical life contingencies book \cite{bowers1997actuarial}. Since life contingencies 
theory grounds on demography and classical financial mathematics, I have made use of the Ruckman and Francis \cite{mathFinAct} and 
...
\begin{enumerate}
	...
	regarding life contingencies theory.
	\item Section \ref{sec:structure} overviews the general structure of 
	the package.
	\item Section \ref{sec:examples} gives a wide choice of 
	\pkg{lifecontingencies} packages example.
	\item Finally section \ref{sec:discussion} will provide a 
	discussion of results and further potential
	developments.
\end{enumerate}

\section{Life contingencies statistical and financial foundations}\label{sec:statistics}

Life insurance analysis involves the calculation of expected values of 
future cash flows, whose probabilities depend by events related to insured life 
contingencies and time value of money. Therefore life insurance actuarial mathematics grounds itself on concepts 
derived from demography and theory of interest (like present value).\\

A life table (also called a mortality table or actuarial table) is a table
that shows, for each age $x$, the number of subjects $l_x$ that are expected to be in life (at risk) at the beginning of age $x$. It represents a sequence of $l_0, l_1, \ldots, l_{\omega}$ being $\omega$ the farthest age that a person can obtain for the life table cohort. A life table is referred to a cohort of subjects, typically varying by sex, year of birth and country.\\

Many statistics can be derived from the $l_x$ sequence. A non exhaustive list follows:
\begin{itemize}
	...
	\item $e_x$, the expected remaining lifetime for someone living at age $x$.
\end{itemize}
An exhaustive coverage of life table demographics can be found in \cite{keyfitz2005applied}.
Life table are usually produced by institutions that have access to large amount of reliable historical data, like official statistics or social security bureaus. Actuaries often start from those table and modify underlying survival probabilities to make the table better fit to the insured pool experience.
Life table analysis provides the empirical data to assess the time - until - death random variables for any individual aged $x$, being $T_x$ and $K_x$ the continuous and curtate form respectively.\\

Classical financial mathematics deals with monetary amount that could be available in different times. The most important concept in classical financial mathematics is the present value (see formula \ref{eq:CF}). Present value represents the current value of a series of monetary cash flows, $CF_t$, that will be available in different periods of time.\\ The interest rates, $i_t$, represents the measure of price of money per unit of time. Formula \ref{eq:interest} shows the relationship between the accumulation function, $A \left(t\right)$ and interest and discount rates, both effective and nominal.\\
...
All financial mathematics functions (as annuities, $\bar{a}_{\lcroof{n}}$, or accumulated values, $s_{\lcroof{n}}$) can be written as a particular case of formula \ref{eq:CF}.\\
...
Actuaries uses the probabilities inherent the life table to evaluate life contingencies insurances. Life contingencies are themselves stochastic variables, in fact. They consist in present values whose amounts are not certain, but the time and the final values depend by events regarding the life of the insured head. Their expected value is named actuarial present value (APV). While APV is certainly the most important statistic used by actuaries, as long as it represents the average cost of the coverage the insurer provides, \pkg{lifecontingencies} provides functions to assess the distribution of life contingencies quantities as random value generation.  

Some examples of life contingencies follow. The term life insurances  represent a contract where a sum $b_t$ is payable whether the insured head dies within n years. A flat n - term life insurance is expressed in formula \ref{eq:termins} and its APV symbol is $\termins{x}{n}$.

\begin{equation}
	Z_n = \left\{ \begin{array}{l}
	{b_{\tilde T}}{v^{\tilde T}}\,\tilde T \le n\\
	0\,\tilde T > n
	\end{array} \right.
\label{eq:termins}
\end{equation}

Another example is the annuity due, $\ddot{a}_x$, that consists in a series of cash flows of equal amounts payable at the beginning of each period until death. Formula \ref{eq:anndue} expresses its APV.

\begin{equation}
	\sum\limits_{k = 0}^\infty  {{a_{\left. {\overline {\, 
	 {k + 1} \,}}\! \right| }}_k{p_x}{q_{x + k}}}
\label{eq:anndue}
\end{equation}

The pure endowment is a stochastic variable defined in formula \ref{eq:pureEnd}. Its APV symbol is $\pureendc{x}{n}$. Under a pure endowment contract a sum is paid after n year if the insured aged $x$ still lives at age $x+n$.
...
The \pkg{lifecontingencies} package provides functions that allows the user to evaluate standard life insurance contract APV. Function for $A_{x}$ (life insurance), $\pureendc{x}{n}$ (the pure endowment), $\ddot{a}_x$ (the annuity due), $\lcterm{(DA)}{x}{n}$ (the decreasing term life insurance) and 
$(IA)_{x}$ (increasing term life insurance) are available as long as variants. Most important variants consist in allowing fractional terms and temporary duration.\\ .

In general present values can be expressed as the scalar product of full value cash flows $\bar c$ and their corresponding discount rate $\bar v$. APV formula taxes into account the present value. While most financial mathematics and lifecontingencies formulas can be expressed in symbolic form, \pkg{lifecontingencies} will evaluate present values and APVs using vector calculus. Therefore \pkg{lifecontingencies} builds the vectors of payments, $\bar{c}$, discounts, $\bar{v}$ and probabilities, $\bar{p}$ and it evaluates their scalar product as shown by equation \ref{eq:coreFormula}.\\  
...
The \pkg{lifecontingencies} package allows actuaries to perform financial and life contingencies actuarial mathematics within \proglang{R}. It offers the basic tools to manipulate life tables and perform financial calculations. Pricing, reserving and stochastic evaluation of most important life insurance contract can be performed within \proglang{R}.\\

Future work spans in multiple directions. Currently \pkg{lifecontingencies} handles only one - year life tables and single causes of decrement only. We expect to make the package more flesible from this side. The stochastic calculation modules will be improved, morover. Similarly we expect to integrate \proglang{C++} fragments using \pkg{Rcpp} package whether it effectively improves performance.\\
Finally we wish to provide \pkg{lifecontingencies} coerce methods toward packages specialized in demographic analysis, like \pkg{demography} and \pkg{LifeTables} and interest rates modelling. 
