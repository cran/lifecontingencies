...
As of May 2013, the \pkg{lifecontingencies} package
\citep{spedLifeCon} appears as the first \proglang{R} package that
deals with life contingent actuarial mathematics. The \proglang{R}
...
Christophe Dutang, Tim Riffle and Reinhold Kainhofer for their suggestions on
code and vignettes. A special thank also to Anton Sylchenko for the
