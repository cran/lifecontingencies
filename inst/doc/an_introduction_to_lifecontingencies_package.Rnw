...
\url{https://cran.r-project.org/package=lifecontingencies}. The examples used throughout this
...
portfolio that will not be impacted by changes in the investment yield. In order to solve
...
\subsection{Visualising life tables}

Life tables can be visualized directly using the \code{autoplot()} method
provided by the package. The method follows the \code{autoplot()} interface
used by \pkg{ggplot2} \citep{Wickham2016ggplot2}, and returns a
\code{ggplot} object. This makes it possible to obtain a graphical
representation of the number of survivors at each age while retaining the
usual \pkg{ggplot2} facilities for further customization.

For example, the following code creates a simple life table and plots the
number of survivors as a function of age:

<<autoplot-lifetable, fig=TRUE, echo=TRUE>>=
library(lifecontingencies)
...
The same method is available for \code{actuarialtable} objects because
\code{actuarialtable} is defined as a subclass of \code{lifetable}.
Consequently, no separate plotting method is required for actuarial tables:

<<autoplot-actuarialtable, fig=TRUE, echo=TRUE>>=
ggplot2::autoplot(soa08Act)
@

The plot focuses on the demographic component of the table, namely the
survivor function represented by $l_x$. The interest rate stored in an
\code{actuarialtable} affects actuarial calculations but does not alter this
basic demographic visualisation. Additional arguments supplied through
\code{...} are passed to \code{geom_line()}, allowing the graphical
appearance of the curve to be customised using standard \pkg{ggplot2}
syntax.

The method therefore provides a convenient bridge between the
life-contingencies classes of the package and the broader \pkg{ggplot2}
...
make the insurer to suffer an underwriting loss with 10\% probability,
would be
...
Another example has been inspired by data found in \cite{de2016assicurazioni}. We use life tables from the quoted book and the following functions (still based on single decrement tables) to compute the APV of (constant or variable) annuity with multiple decrement. 
...
\section{Software engineering and package architecture}
\label{sec:software-engineering}

Although \pkg{lifecontingencies} is primarily presented as a collection of
actuarial and demographic functions, its implementation follows the standard
architecture of an \proglang{R} package and combines interpreted
\proglang{R} code with compiled code where computational efficiency is
important.  This separation makes it possible to maintain a high-level,
user-facing \proglang{R} interface while implementing selected numerical
operations in compiled \proglang{C++} code.

The main components of the source tree can be summarized as follows:
...
lifecontingencies/
  ...
  tests/
    testthat.R          entry point for package tests
    testthat/           automated test suite
  inst/
    benchmarks/         performance-oriented development benchmarks
  vignettes/             package documentation and examples
\end{verbatim}

The \proglang{R} directory contains the public-facing functions and the
implementation of the package's actuarial and demographic classes.  The
\proglang{C++} code in \code{src/} is used for selected computationally
intensive operations.  The interface between \proglang{R} and \proglang{C++}
is provided by the \pkg{Rcpp} package \citep{Eddelbuettel2011Rcpp}.  This
approach allows performance-critical numerical computations to be compiled
while retaining the \proglang{R} API used by package users.

The compiled backend is treated as an implementation detail rather than as a
separate user-facing API.  Consequently, changes to the computational backend
can be validated against the established \proglang{R} implementations and
against the package's actuarial identities and numerical properties.  When an
independent historical \proglang{R} implementation is available, regression
tests can use it as a reference implementation.  This is particularly useful
when replacing a frequently called \proglang{R} computation with a compiled
implementation: the objective is to improve execution time without changing
the mathematical result.

Automated testing is organized using the \pkg{testthat} framework
\citep{Wickham2011testthat}.  The files under \code{tests/testthat/} contain
functional, regression, validation, and defensive-programming tests.  The
test suite is executed as part of the package checking and continuous
integration process.  This provides an additional safeguard against
unintentional changes in numerical results or in the public behaviour of the
package.

Performance measurements are kept separate from the functional test suite.
Benchmark scripts are useful during development for comparing alternative
implementations, in particular interpreted \proglang{R} and compiled
\proglang{C++} implementations, but execution time is inherently dependent on
the hardware and software environment.  Such measurements are therefore not
used as pass/fail criteria for the standard package tests.

This architecture separates four complementary concerns: the actuarial
interface and mathematical implementation in \proglang{R}, computationally
intensive numerical kernels in \proglang{C++}, automated correctness checks
through \pkg{testthat}, and performance evaluation through dedicated
benchmarks.  Together with continuous integration, this structure supports
the long-term maintenance of the package while allowing computational
optimizations without changing its user-facing interface.
...
continuously. Key parts of core functions use \proglang{C++} code
snippets via the \pkg{Rcpp} package \cite{RcppR}.
