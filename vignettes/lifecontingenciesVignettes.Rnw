...
As of March 2012, \pkg{lifecontingencies} appears to be the first \proglang{R} package that deals with life insurance evaluation.
...
We remaind to the \cite{bowers1997actuarial} manual for formulas regarding other life contingencies insurances as $\lcterm{(DA)}{x}{n}$, the 
decreasing term life insurance, $(IA)_{x}$, the increasing term life insurance, and common variations on benefit payments: deferrment and fractional amounts.
...
useless in computer era since it is not enough flexible and it is computationally inefficient. Therefore commutation table 
approach has not been used within \pkg{lifecontingencies} to perform APV calculations.\\
The current payment technique calculates the APV, $\bar z$, as the scalar product of three vectors: $\bar z = \left\langle {\left\langle {\bar c \bullet \bar v} \right\rangle  \bullet \bar p} \right\rangle$. The vector of uncertain cash flows, $\bar c$, 
the vector of discount factors, $\bar v$ and the vector of cash flow probability. Since the current payment technique is the the most efficient approach from a computationally side perspective, 
we have used this approach to evaluate APV. Finally, the expected value approach models the APV as the scalar product of two vector: $\Pr \left[ {\tilde K = k} \right]$, 
...
Class \code{actuarialtable} inherits from \code{lifetable} class. It has an additional slots for the interest rate.
...
\begin{itemize}
	...
	\item \code{i}, the interest rate that overrides the \code{actuarialtable} default interest rate.
...
The \pkg{lifecontingencies} package provides functions for classical financial mathematics calculation.\\
...
Following examples show how to switch from effective interest rates (APR) to nominal 
interest rates, that is $i^k \to i$ and vice versa.
...
\code{real2Nominal} and \code{nominal2Real} work with discount rates also, that is they allows $d^(m) \to d$ and 
vice versa.


<<discount rates functions>>=
...
<<present value>>=
#suppose an investment requires an initial outflows of 1000 
#and suppose its projected inflows to be 200, 500 and 700
capitals=c(-1000,200,500,700)
#at times.
times=c(0,1,2,5)
#the net prepreset value of the investment is 
...
#e.g. 4% 2% 3% 5.7%
iRates=c( 0.04, 0.02, 0.03, 0.057)
presentValue(cashFlows=capitals, timeIds=times, 
             interestRates=iRates)
#and if the last cash flow were uncertain, as we assume a 
#probability to receive it of 50%, all other being equal 
#the expected APV is 
presentValue(cashFlows=capitals, timeIds=times, 
interestRates=iRates, 
...
Code below shows examples of annuities ($a_{\left. {\overline {\, n \,}}\! \right| }$) and accumulated values ($s_{\left. {\overline {\, n \,}}\! \right| }$) 
evaluations.
...
While a more real-life example is shown below
...
#0.05 is paid, how much does the man need to save each year (R) in order to
#meet his target?
C=100000
...
while the code below shows how fractional annuities ($a_{lcroof{n}}^{(m)}$) can be handled when using 
...
#An increasing n-payment annuity-due shows payments of 1, 2, ... , n
#at time 0, 1, ... , 
...
#assuming each year the annuity increases its value by 3%
#being the interest rate is 4%
#and the annuity duration being 10 years
...
The code below shows an investment amortization payment schedule.
Suppose loaned capital is $C$, then assuming an interest rate i, 
the amount due to the lender at each instalment is $R = \frac{C}{a_{\left. {\overline {\, n \,}}\! \right| }}$.
At each installment the $R_t$ amount repays $I_t=C_{t-1}*i$ as interest and 
$C_t=R_t-I_t$ as capital.

<<capital amortization>>=
#assumeing: 5% annual interest rate
...
installment=1/payments_per_year*capital/annuity(i=interest, n=years,k=payments_per_year)
installment	
...
#using functions provided within lifecontingencies package
...
\code{lifetable} and \code{actuarialtable} classes are designed to handle demographics and actuarial mathematics 
calculations. A \code{actuarialtable} class inherits from \code{lifetable} class. It has one more slot that sets the 
rate of interest. Both classes have been designed using the S4 R classes framework.\\
Following examples show how these classes are initialized, basic survival probabilities and life tables analysis. 
...
\begin{enumerate}
	...
	starts from zero and going to the $\omega$ age (the age $x$ that $p_x=0$).
	\item The $l_x$ vector, that is 
	the number of subjects living at the beginning of age $x$.
	\item The name of the life table.
\end{enumerate}

To create a \code{lifetable} object directly we can do as follows
...
\code{print} and \code{show}  methods are available that tabulate 
the x, lx, px and ex values.
...
from a suitable existing \code{data.frame}.
...
The last way a \code{lifetable} object can be created is to generate it from 
...
with linear interpolation, constant force of mortality and hyperbolic assumption (see 
\cite{bowers1997actuarial} for details).\\

<<fractional ages, keep.source=TRUE>>==
#using Society of Actuaries illustrative life table
data(soa08Act)
...
Calculations of two heads survival probabilities can be performed also, as code below
shows:

<<more than one head, keep.source=TRUE>>=
#using example life tables
pxyt(exampleLt,exampleLt,x=6, y=7, t=2) #joint survival probability
pxyt(exampleLt,exampleLt,x=6, y=7, t=2,status="last") #last survival probability

#using two distinct real life tables (Italian IPS55 tables for males and females)
#evaluate the expected joint life time for a couple aged 65 and 63  respectively
...
in most \cite{bowers1997actuarial} examples) will be used, unless otherwise stated. 
...
while following codes evaluate pure endowments APV, $\pureendc{x}{n}$
...
Now we will evaluate the benefit reserve for a 20 year l
ife insurance of 100,000, whith benefits payable at the end of year of death, whith level benefit premium payable at the beginning of each year. Assume 3\% of interest rate and SOA life table to apply.\\
The benefit premium is $P$, determined by equation $P \anndue{40}{20} = 100000 \lcterm{A}{40}{20}$, while 
the benefit reserve is determined by equation ${}_{k}\lcterm{V}{40+t}{n-t}=100000\lcterm{A}{40+t}{20-t}-P\anndue{40+t}{20-t}$ for 
$t = 0 \ldots 19$.
...
distribution is implicit within the deaths tabulated within a life table.
...
amount and indicator variables regarding the life status of the insured. This is the present value of benefits 
random variable, $\tilde Z$.\\
Usually life contingencies evaluation functions return the APV, as the \code{type} parameter
has "EV" (expected value) as default value. However most life contingencies insurance evaluation 
functions are provided with a "ST" parameter that allows to obtain a variate from the underlying 
present value benefit distribution.\\
We will apply these concepts to term life insurance with code below:

<<Axn APV and stochastic,keep.source=TRUE>>=
#term life insurance 25 years old, 40 year term
#APV
APV=Axn(soa08Act,x=25,n=40,type="EV")
APV
#analysis of the distribution
sampleAxn=numeric(1000)
for(i in 1:1000) sampleAxn[i]=Axn(soa08Act,x=25,n=40,type="ST")
summary(sampleAxn)
t.test(x=sampleAxn,mu=APV)$p.value
#unbiased
@

to increasing life insurance

<<IAxn APV and stochastic,keep.source=TRUE>>=
#increasing life insurance 25 years old, 40 year term
#APV
APV=IAxn(soa08Act,x=25,n=40,type="EV")
APV
#analysis of the distribution
sampleIAxn=numeric(1000)
for(i in 1:1000) sampleIAxn[i]=IAxn(soa08Act,x=25,n=40,type="ST")
summary(sampleIAxn)
#verify if sampleIAxn expected value is APV
t.test(x=sampleIAxn,mu=APV)$p.value
#unbiased
@

Current \pkg{lifecontingencies} allows to obtain stochastic sample of present value benefits function 
for all the one head life contingencies insurances. \code{rLifeContingencies} function allow
to obtain a sample of size $n$ from any life contingency insurance.

<<axn sample, keep.source=TRUE>>=
#sample from the distribution of an annuity due
#x=65
simAnnuity<-rLifeContingencies(n=1000, lifecontingency="axn",object=soa08Act,
x=65,t=getOmega(soa08Act)-65, i=soa08Act@interest,m=0,k=1)
@



Figure \ref{fig:a65} shows the distribution of simAnnuity vector, whose APV is $\ddot{a}_{65}$.
